...
struct sock_filter filter[] = {
    ...
    Allow(read),        /* read sys_call*/
    Allow(exit),        /* exit sys_call*/
    Allow(execve),
    Allow(fork),
    Allow(close),
    Allow(brk),
...
void command(char* par2[], char const *argv[]) {
  //printf("%s\n", par2[0]);
  struct rlimit limit;
  int z;
  pid_t pid;
  ...
  if (pid < 0){
    ...
    char *d= argv[1];
    if(strcmp(argv[1], "-r")==0 || strstr(d, "r"))
    {
    if(argv[2]!=NULL)
    {
      limit.rlim_cur =atoi(argv[2]);
      limit.rlim_max =atoi(argv[2])+1;
      setrlimit(RLIMIT_CPU, &limit);

      limit.rlim_cur = atoi(argv[3])*1000000;
      limit.rlim_max = (atoi(argv[3])+1)*1000000;
      setrlimit(RLIMIT_AS, &limit);

      limit.rlim_cur = atoi(argv[4])*1000000;
      limit.rlim_max = (atoi(argv[3])+1)*1000000;
      ...
      limit.rlim_cur = 16*1000000;
      limit.rlim_max = 17*1000000;
      setrlimit(RLIMIT_AS, &limit);

      limit.rlim_cur = 1*1000000;
      limit.rlim_max = 2*1000000;
      setrlimit(RLIMIT_STACK, &limit);
  ...
  err(0, "setuid failed.");
    ...
    ptrace(PTRACE_TRACEME, 0, 0, 0);
        ...
        if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0)) {
              perror("Could not start seccomp1:");
              exit(1);
          }
          /* set up the restricted environment */
        if (prctl(PR_SET_SECCOMP, SECCOMP_MODE_FILTER, &filterprog) == -1) {
              perror("Could not start seccomp2:");
              exit(1);
          }

    if(execl(par2[0], par2, NULL) < 0){
    ...
    while(1){
      ...
      //  if (banned_call(orig_eax)){
          //fprintf(stderr, "Trying to access banned system call #%ld. Killing.\n", orig_eax);
          //kill(child, SIGKILL);
      //  }
    // wait4(0,&status, 0, &usage);
    ...
    while ( *p != '\0' ) {
      ...
      if (*p=='c') {
        printf("USER Time : %lld.%llds\n", usage.ru_utime.tv_sec, usage.ru_utime.tv_usec);
      }
      if (*p=='m') {
        printf("Operating residential memory : %lld\n", usage.ru_maxrss);
      }
      if (*p=='b') {
        printf("BIO : %ld\n", usage.ru_inblock);
        printf("BOO : %ld\n", usage.ru_oublock);
...
int main(int argc, char const *argv[]) {
  ...
  chdir("/home/altun/Desktop/jail");
  if(chroot("/home/altun/Desktop/jail")<0)
  {
    perror("Chroot error");
  }
  printf("%s\n", get_current_dir_name());
